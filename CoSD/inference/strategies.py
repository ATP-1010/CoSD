...
def _MCSSwoReplacement(
    ...
    # CoSD-Rule
    cand_probs = cand_probs.to(ground_probs.device)
    ground_token = torch.argmax(ground_probs).item()

    for check_idx, cand_token in enumerate(cand_tokens):
        if cand_token == ground_token:
            continue

        g_prob = ground_probs[cand_token]
        c_prob = cand_probs[cand_token]

        if c_prob > 0.5 * g_prob and g_prob < 0.5:      ## alpha and beta here
            return check_idx
        else:
            # reject
            ground_probs -= cand_probs
